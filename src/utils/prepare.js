...
const transformNumber = (num, digits = 2) => {
  return (
    !Number.isInteger(num) ? num.toFixed(digits) : num.toString()
  ).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
};

const transformObject = (value) => (!Object.keys(value).length ? null : value);

const transformArray = (value) => (!value.length ? null : value);
...
const filterPassedTime = (from) => (to) => {
  const dateFrom = new Date(from);
  const dateTo = new Date(to);
  return dateTo.getTime() > dateFrom.getTime();
};

const onExportFile = (file, fileName) => {
...
const transformToCapitalizeCase = (str) => {
  return str
    .toLowerCase()
    .split(' ')
